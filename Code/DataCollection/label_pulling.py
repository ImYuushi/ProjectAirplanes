...
"""
def html_paths(year):
    return config.data_path.joinpath('Websites').joinpath('comet_tables').joinpath(f'comet_table_{year}.html')
def url_path(year):
    return "https://sungrazer.nrl.navy.mil/comets_table_"+ str(year)
def comet_path():
     return config.data_path.joinpath('Websites').joinpath('comet_data')
...
def parse_all_comets():
    ...
    df.to_csv(config.data_path.joinpath('csvs').joinpath("raw_confirmation_data.csv"))
...
def collect_labels(download_data=False):
    if download_data:
