...
def gen_urls(year,camera):
    link_save_path = (Path(__file__).parent).joinpath("Image_Links").joinpath(f"img_urls_{camera}_{year}.txt")
...
def download_imgs(year,camera):
    link_save_path = (Path(__file__).parent).joinpath("Image_Links").joinpath(f"img_urls_{camera}_{year}.txt")
    img_save_path_base = (Path(__file__).parent).joinpath("images_dl").joinpath(f"images_{year}_{camera}")
...
def create_img_csv(year, camera):
    csv_save_path = (Path(__file__).parent).joinpath("tables").joinpath(f"image_data_{year}_{camera}.csv")
    link_save_path = (Path(__file__).parent).joinpath("Image_Links").joinpath(f"img_urls_{camera}_{year}.txt")
