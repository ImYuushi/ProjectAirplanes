...
def main(years,cameras):
    df_labels = pd.read_csv(data_path.joinpath("Labels").joinpath("all_comet_confirmations.csv")).drop(['Unnamed: 0'], axis=1)
    ...
    for year in years:
        for camera in cameras:
            ...
            if df_img.empty:
                df_img = pd.read_csv(data_path.joinpath("Images").joinpath(f'image_data_{year}_{camera}.csv'),dtype={"Unnamed: 0": int, "Date" : str, "Time" : str})
                df_img = df_img.drop(['Unnamed: 0'], axis=1)
                df_img['Tel'] = camera
                df_filtered_labels = df_tmp
            else:
                df_tmp_img = pd.read_csv(data_path.joinpath("Images").joinpath(f'image_data_{year}_{camera}.csv'))
    ...
    # print(df_img[df_img['Confirmations'].isna()])
    for _, row in tqdm(df_filtered_labels.iterrows()):
    # for _, row in (df_filtered_labels.iterrows()):
        row_time = row['Time']
        row_date = row['Date']
        tel = row['Tel']
        ...
        d = row_date.replace("-", '')
        # print(f'T: ({t}), D: ({d})')
        # print(df_img[(df_img['Time'] == t) & (df_img['Date'] == d)])
